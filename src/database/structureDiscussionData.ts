...
export async function setDiscussion(discussions) {
  const discussionsIds = discussions.map(
    (discussion) => structureDiscussionData({ discussion })?.id
  );
  ...
  data.forEach(async (discussion, index) => {
    ...
    if (discussion) {
      ...
      newDiscussions.push(
        structureDiscussionData({
          discussion: remoteDiscussion,
        })
      );
    const { newMessages: _newMessages, updateMessages: _updateMessages } =
  ...
  if (newMessages.length) await db?.messages.bulkAdd(filterByKey(newMessages));
