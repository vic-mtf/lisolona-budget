...
import useLocalStoreData from "../../../hooks/useLocalStoreData.js";
...
export default function AudioInputButtonOptions() {
  ...
  const deviceIdSelected = useMemo(
    () =>
      devicesRef.current?.find(({ deviceId }) => device?.deviceId === deviceId)
        ?.deviceId,
    [device]
  );
    ...
    async (newDevice) => {
      ...
      if (device?.deviceId !== newDevice.deviceId) {
        try {
          ...
          if (audioTrack) audioTrack.enabled = micro.active;
        } catch (e) {
          dispatch(
            setAudioDevice({
              type: "input",
        ...
        _devices
          ...
          .forEach((device) => {
            if (!devices.find(({ deviceId }) => device.deviceId === deviceId)) {
              if (device?.deviceId?.trim()) audioInputs.push(device);
            }
          });
        const [data] = devices;
    ...
    <React.Fragment>
      ...
      <Menu
        ...
        <MenuItem
          ...
          <ListItemText>
            {micro.active ? (
              <AudioLevelIndicator stream={audioStreamRef.current} />
            ) : (
