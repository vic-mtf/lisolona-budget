...
export default function useGetData (callback) {
    ...
    const handleMessages = useCallback(async () => {
        ...
        data.forEach(message => {
            const index = messageGrouping.findIndex(
                ({targetId}) => targetId === message.targetId
            );
            if(index === - 1) 
                ...
                if(messageGrouping[index]?.messages.length < 15)
                messageGrouping[index].messages.push(message)
            }   
        });
        ...
        const onGetData = async ({chats}) => {
            ...
            chats?.forEach((discussion) => {
                ...
                const localDiscussion = {
                    ...
                    discription: discussion.discription,
                    origin: discussion,
                };
    ...
    }, [socket, userId, handleMessages, online]);
}