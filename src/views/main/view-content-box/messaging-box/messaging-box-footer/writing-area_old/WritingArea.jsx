...
const WritingArea = React.memo(({ onSubmit, target, media }) => {
  ...
  const placeHolder = useMemo(
    () =>
      `Entrez votre message: ${target?.type === "room" ? "#" : "@"}${
        target?.name
      }...`,
    [target]
  );
  const handleSubmit = useCallback(() => {
    ...
    const HTML = isText
      ? draftToHtml(rawContentState, {}, true, customEntityTransform)
      : null;
  ...
  return (
    <Paper
      ...
      <MuiBox ref={rootRef} component='div' sx={publicDraftStyle}>
        <TextEditorMessage
          paperRef={paperRef}
          filesRef={filesRef}
          rootRef={rootRef}
