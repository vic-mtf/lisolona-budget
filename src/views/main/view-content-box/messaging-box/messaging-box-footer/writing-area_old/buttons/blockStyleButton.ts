...
export default function BlockStyleButton(props) {
  const { icon, label, value, getEditorState, setEditorState, ...otherProps } =
    props;
  ...
  return (
    <>
      <ClickAwayListener onClickAway={() => (open ? setOpen(false) : null)}>
        <div style={{ position: "relative" }} className={otherProps.className}>
          <ToggleButton
            {...otherProps}
            value=''
          ...
          <ElasticPopper
            ...
            <CustomPaper
              sx={{
                ...
                boxShadow:
                  "0px -3px 5px  rgba(0,0,0,0.2),0px -5px 5px rgba(0,0,0,0.14),0px -10px 14px transparent",
                mx: 0.5,
