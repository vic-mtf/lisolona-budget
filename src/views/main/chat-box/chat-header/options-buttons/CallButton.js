...
export default function CallButton({ target, theme }) {
  ...
  const HandlerInitCall = useCallback(
    (action, props) => () => {
      setAnchorEl(null);
    ...
    [secretCode, socket, target]
  );

  return (
    <React.Fragment>
      <Tooltip
        title={disabled ? "Un appel en cours..." : "Lancer l'appel"}
        arrow>
        <div>
          <IconButton
            sx={{ mx: 1 }}
            ...
            onClick={() =>
              target?.type === "room"
                ? onOpen()
                : HandlerInitCall(options[0].action, { mode: "outgoing" })()
            }>
