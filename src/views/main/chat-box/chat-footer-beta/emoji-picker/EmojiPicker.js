...
export default function EmojiPicker({ onSelect }) {
  ...
  const data = useMemo(
    () => getEmojisData(group, color, style),
    [group, style, color]
  );
  ...
  return (
    <MuiBox display='flex' width='100%'>
      <Paper
        ...
        <Stack divider={<Divider />} ref={headerRef}>
          <EmojiPickerHeader
            ...
            key={nButton ? 0 : 1}
          />
          <EmojiPickerContent onSelect={onSelect} data={data} key={group} />
        </Stack>
      </Paper>
    </MuiBox>
  );
...
export const getEmojisData = (
  ...
  const data = _ASSETS_INFO.data.filter(
...
export function getCharacterFromCodeString(codeString) {
  return codeString.replace(/\\u([a-fA-F0-9]{4})/g, (match, group) =>
    String.fromCodePoint(parseInt(group, 16))
  );
}
