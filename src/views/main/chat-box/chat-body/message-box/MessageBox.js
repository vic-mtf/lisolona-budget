import { Box as MuiBox, Paper, Stack, useTheme } from "@mui/material";
...
const MessageBox = React.memo(
  ({
    ...
    const bgColor = useMemo(
      () => (isMine ? theme.palette.grey[200] : theme.palette.background.paper),
      [isMine, theme]
    );

    return (
      <MuiBox component='div' style={{ marginTop: joinBox ? "1px" : "5px" }}>
        <MuiBox display='flex'>
          <Stack
            ...
            flex={1}>
            {!isMine && (
              <MuiBox display='flex' alignItems='end' width={25}>
                {!hideAvatar && (
                  <Avatar
                    sx={{
                      width: 25,
