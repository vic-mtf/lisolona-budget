...
export default function MessageButton({ getVideoStream }) {
  ...
  useEffect(() => {
    ...
    const handleAddMessageCounter = () => {
      ...
      newMessageSignal.audio.volume = 0.5;
    };
    if (!selected)
      MESSAGE_CHANNEL.addEventListener(name, handleAddMessageCounter);
    if (messagesCounter && selected) setMessagesCounter(0);
    return () => {
      MESSAGE_CHANNEL.removeEventListener(name, handleAddMessageCounter);
    };
