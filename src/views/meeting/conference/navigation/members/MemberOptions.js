...
export default function MemberOptions({ rootRef, id, name, state }) {
  ...
  const { isOrganizer } = useClientState({
    id: me?.id,
    props: [],
    key: "state",
  });
  ...
  const options = useMemo(() => {
    ...
    if (isOrganizer) data = data.concat(adminOptions(states, socket));
    return data;
    ...
    <>
      <Stack spacing={1} direction='row'>
        <RaiseHandView
          ...
          id={id}
        />
        <MicroOption active={Boolean(tracks[1])} id={id} />
        {options.length > 0 && (
          <MoreOptions
...
const adminOptions = (state, socket) => {
  ...
  return [
    {
      label: isMic ? "Désactiver le micro" : "Activer le micro",
    ...
    {
      label: isCam ? "Désactiver la caméra" : "Activer la caméra",
      disabled: isMine
        ? loadingCam
        : !state?.videoTrack && user?.id !== state?.id,
      icon: isCam ? (
    ...
    },
    {
      label: state?.isOrganizer
        ? "Retirer comme modérateur"
        : "Désigner comme modérateur",
