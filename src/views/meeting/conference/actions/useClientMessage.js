...
export default function useClientMessage () {
    ...
    useEffect(() => {
        ...
        const getDiscussion = () => {
            ...
            const type = ( target.type === 'room' || participants.length > 2 ) ? 'room' : 'direct';
        ...
        const handleGetMessage = ({message, where}) => {
            ...
            if(where?._id === discussion?._id) {
                const index =  meetingMessagesRef.current.findIndex(({id}) => id === message.clientId);
                ...
                if(index !== -1) {
                    ...
                    meetingMessagesRef.current[index] = mergeDeep(
                        currentMessage, 
                        localMessage
                    );
