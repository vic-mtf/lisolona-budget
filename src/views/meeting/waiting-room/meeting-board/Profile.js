...
export default function Profile({ handleAskJoinMeeting }) {
  ...
  return (
    <Stack alignItems='center' display='flex' spacing={2}>
      ...
      <Button variant='contained' onClick={handleAskJoinMeeting}>
        Joindre la réunion
      </Button>
    </Stack>
  );
...
const CustomAvatar = ({
  ...
  const { background, text } = useMemo(
    () => generateColorsFromId(id, theme.palette.mode),
    [theme, id]
  );
...
function participantsAppel(participants) {
  ...
  if (participants.length > 2) {
    ...
    message = `${participants[0]}, ${participants[1]} et ${others} ${
      others === 1 ? "autre" : "autres"
    }`;
