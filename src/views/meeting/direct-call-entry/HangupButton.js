...
export default function HangupButton({ setCallState }) {
  ...
  const [{ streams, client }] useLocalStoreData();
      ...
      <ThemeProvider
        ...
        <Fab
          ...
          onClick={async () => {
            ...
            streams.forEach(
              async (stream) => await closeMediaStream(stream.current)
            );
            if (store.getState().meeting.joined) await client.leave();
        ...
        <Typography variant='caption' align='center' fontSize={10.5} noWrap>
          Racrocher
        </Typography>
      </ThemeProvider>
    </Stack>
