...
const ListItems  = ({contacts, search}) => {
    ...
    const {filteredObjects, letters, countByLetter } = useMemo(() => 
        processObjects(contacts , search), [contacts, search]
    );
    ...
    const itemContent = useCallback ((index) => {
        ...
        const isDivisible = Boolean(next) && 
        next.name.charAt(0).toLowerCase() ===  contact.name.charAt(0).toLowerCase();
...
function processObjects(objects=[], search='') {
    const sortedObjects = objects.map(obj => ({
      ...obj,
      name: `${obj.firstName} ${obj.middleName ? obj.middleName + " " : ""}${obj.lastName}`,
    ...
    const countByLetter = letters.reduce((acc, letter) => {
      const count = filteredObjects.filter(obj => obj.name.charAt(0).toUpperCase() === letter).length;
      acc[letter] = count;
      return acc;
    }, {});
