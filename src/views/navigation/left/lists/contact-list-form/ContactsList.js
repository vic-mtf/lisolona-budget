...
export default function ContactsList({
  ...
  const filterByKey = useCallback(
    (key) =>
      new RegExp(escapeRegExp(search?.trim().split(/\s/).join("|")), "ig").test(
        key
      ),
    [search]
  );
  ...
  const contacts = useLiveQuery(
    () =>
      db?.contacts
        .orderBy("name")
        .filter(({ name, email }) => filterByKey(name) || filterByKey(email))
        .reverse()
        .toArray(),
    [filterByKey]
  );
  ...
  const handleSelected = (contact) => () => {
    setItemsSelected((_items) => {
      ...
      if (itemListRef?.current) itemListRef.current = items;
      return items;
    });
  };
          ...
          {contacts?.map((contact, index, contacts) => (
            <React.Fragment key={contact.id}>
              <ContactItem
                {...contact}
                selected={Boolean(
                  itemsSelected.find((item) => item.id === contact.id)
                )}
                onClick={
                  mode === "contact"
