...
export default function ContactListForm() {
  const [{ open, mode, externalErrors, step, next }, setValues] = useState({
    step: 0,
  });
  ...
  const handleSubmit = (event) => {
    ...
    if (!groupTitleRef.current?.trim()) externalErrors.push("title");
    if (!descriptionRef.current?.trim()) externalErrors.push("description");
  ...
  useEffect(() => {
    ...
    root.addEventListener("_auto_open_create_group", handleAutoOpen);
    return () => {
      root.removeEventListener("_auto_open_create_group", handleAutoOpen);
    };
  }, []);
            ...
            <Toolbar variant='dense' disableGutters>
              {mode !== "group" && (
                <IconButton
                  sx={{ mr: 1 }}
                  onClick={() =>
                    setValues((values) => ({ ...values, mode: "contact" }))
                  }>
          ...
          {mode === "contact" && (
            <Toolbar disableGutters variant='dense'>
              <Button
                ...
                sx={{ mr: 1 }}
                onClick={() =>
                  setValues((values) => ({ ...values, mode: "semi-group" }))
                }
                fullWidth>
            ...
            {step > 0 ? (
              <React.Fragment>
                <Button
                  ...
                  key='_previous'
                  onClick={() =>
                    setValues((values) => ({ ...values, step: 0 }))
                  }>
                  Précédent
                </Button>
                <LoadingButton
              ...
              mode !== "contact" && (
                <Button
                  ...
                  disabled={!next}
                  onClick={() =>
                    setValues((values) => ({ ...values, step: 1 }))
                  }>
                  Suivant
                </Button>
              )
