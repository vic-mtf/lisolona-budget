...
export default function Multimedia({
  ...
  const handleChange = useCallback(
    (event, newValue) => {
      setValue(newValue);
      if (newValue === items.length - 1 && slideShow) setSlideShow(false);
    },
  ...
  const userInfos = useMemo(() => {
    ...
    if (media) {
      title =
        headerTitle ||
        `envoyé par ${media.isMine ? "vous" : media.name}, ${timeHumanReadable(
          media.createdAt,
          true
        )}`;
    }
  ...
  useLayoutEffect(() => {
    ...
    if (openRef.current && defaultValue < 0) openRef.current = false;
  }, [defaultValue]);
