...
export default function VoiceNoteEqualizer() {
  ...
  useLayoutEffect(() => {
    const handleChangeStreamState = (event) => {
      if (["prompt", "granted"].includes(permissionState)) handleAudioStream();
    };
          ...
          {stream ? (
            ...
            <DialogContent>
              <DialogContentText variant='body2'>
                Veuillez accorder à l'application <b>Lisolo Na Budget</b>{" "}
                l'autorisation d'accéder au microphone de votre appareil afin de
                vous permettre d'enregistrer votre note vocale{" "}
              ...
              <DialogActions sx={{ py: 0 }}>
                ...
                {permissionState === "prompt" && (
                  <Button variant='outlined' onClick={handleAudioStream}>
                    Autoriser
                  </Button>
                )}
              </DialogActions>
