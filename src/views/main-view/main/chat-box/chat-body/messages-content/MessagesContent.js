...
function MessageContent({ rootRef: _rootRef, target, media }) {
  ...
  const messages = useMemo(
    () =>
      messageGrouping
        .flat()
        .filter(({ type }) => (media ? true : type === "text")),
    [messageGrouping, media]
  );
  ...
  const onloadMessages = useCallback(async () => {
    ...
    const loadingMessages = await getMessages(data);
    setMessageGrouping((messageGrouping) => {
      const updateMessages = replaceObjects(update, messageGrouping?.flat());
