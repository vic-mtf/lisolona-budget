...
export default function InputsCode({
  ...
  const handleChange = (id) => (event) => {
    ...
    if (blurEnd) document.getElementById(`input_code_${read - 1}`)?.blur();
...
function checkStringType(type, string) {
  if (type === "alpha") return /^[a-zA-Z]+$/.test(string);
  else if (type === "alphanum") return /^[a-zA-Z0-9]+$/.test(string);
  else if (type === "num") return /^[0-9]+$/.test(string);
  else return false;
}
