...
export default function useMediaTracks({ type = "", tracks = [] }) {
  ...
  const deleteTrackByUid = useCallback(
    (uid) => {
    ...
    ({ id, uid, track }) => {
      ...
      if (~index && track) {
        const tracks = mediaTracks.tracks.map((_track) => {
          let result = _track;
          if (_track.id === id) result = { ...result, track };
          return result;
        });
        setMediaTracks(({ type }) => ({ type, tracks }));
      }
  ...
  const getTrackByUid = useCallback(
    (uid) => {
  ...
  const getTrack = useCallback(
    ({ uid, id }) => {
      const data = mediaTracks.tracks.find(
        (track) => track.uid === uid || track.id === id
      );
      return data
        ? {
            ...data,
